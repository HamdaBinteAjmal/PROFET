...
;;; function-by-function listing with name, parameters, and
...
(defun gen-overview (system-names)
  ...
  (with-open-file (stream (aima-file 
    ...
    (format t "~&Working on system ~A~%" (setf *aima-system* name))
...
(defmethod print-object ((exp semi-comment) stream)
...
(defun read-semi-comment (stream char)
  ...
  (declare (ignore char))
  (let ((num-semis 1))
    (loop while (eql (peek-char nil stream) #\;) do
      (read-char stream)
      (incf num-semis))
    (if (>= num-semis 3)
   ...
   ((consp exp)
    (let* ((name (first (mklist (second exp))))
	   ...
	   (kind
	    (case (first exp)
	      ((DEFUN DEFGENERIC) 
	       (setf args (or (remove-&aux (mapcar #'first-atom (third exp)))
			      "()"))
	       "function")
	      ((DEFMETHOD) (setf args (third exp)) "method")
	      ((DEFMACRO) (setf args (third exp)) "macro")
	      ((DEFVAR DEFPARAMETER) "variable")
	      ((DEFCONSTANT) "constant")
	      ((DEFTYPE) "type")
	      ((DEFSTRUCT)
	       (setf args (mapcar #'first-atom
